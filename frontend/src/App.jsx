...
function App() {
  ...
  const handleDel = async (id) => {
    const res = await http.del("api/tasks/" + id);
    setTasks((pre) => pre.filter((_task) => _task.id !== id));
    console.log(res);
  };
  const handleCheck = async (id, currentStatus) => {
    const status = !currentStatus;
    const res = await http.patch(`api/tasks/${id}`, {
      isCompleted: status,
    });
    setTasks((pre) => {
      return pre.map((_task) =>
        _task.id === id ? { ..._task, isCompleted: status } : _task
      );
    });
  };
  return (
          ...
          <div className="w-full h-100 flex items-center justify-center">
            <div className="p-2 bg-gray-300 w-90 h-90 overflow-auto rounded-2xl">
              <table className="min-w-full border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                ...
                <tbody id="tableBody" className="divide-y divide-gray-200">
                  {tasks?.map((task) => (
                    <tr key={task.id}>
                      <td className="px-4 py-2">
                        <input
                          ...
                          onChange={() => {
                            handleCheck(task.id, task.isCompleted);
                          }}
                        />
                      </td>
